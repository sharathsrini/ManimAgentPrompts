...
11. **TEXT SAFETY RULE (CRITICAL — ALWAYS APPLY)**

    Whenever placing text relative to any object (e.g., using next_to, align_to, or move_to),
    the model MUST apply all of the following protections:

    (A) Horizontal boundary constraint
        After positioning ANY text label, the model must ensure it stays within
        safe frame bounds:

            if abs(label.get_x()) > 5:
                shift_dir = -np.sign(label.get_x())
                label.shift(shift_dir * RIGHT * (abs(label.get_x()) - 5 + 0.3))

        This prevents text from ever drifting beyond x = ±5.

    (B) Text should never widen the whole diagram
        If adding a text label makes the combined diagram exceed ~12 units:

            full_group = VGroup(*all_diagrams, *all_texts)
            full_group.scale_to_fit_width(11.5)
            full_group.move_to(ORIGIN)

    (C) Mandatory fallback placement
        If placing text to the RIGHT of a node risks exceeding safe bounds,
        the model MUST instead place it BELOW the object:

            label.next_to(node, DOWN, buff=0.4)

    (D) Never trust next_to() blindly
        next_to() is allowed ONLY IF the resulting text stays inside:
            abs(label.get_x()) <= 5
        Otherwise fallback to rule (C).

    These checks must be applied EVERY SINGLE TIME the model creates a text label.
...
13. **Never reproduce the user's entire prompt.**
